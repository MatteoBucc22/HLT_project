...
def train():
    ...
    # Weight decay grouping (exclude bias/LayerNorm)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_idx, batch in enumerate(loop, 1):
            ...
            total_loss += loss.item()
            loop.set_postfix(loss=total_loss / batch_idx)

        avg_loss = total_loss / len(train_loader)
        elapsed = time.time() - start_time
        print(f"\nEpoch {epoch} — Avg Train Loss: {avg_loss:.4f} — Time: {elapsed:.1f}s")
    ...
    # Salva embeddings finali per ensemble
    generate_embeddings(model, val_loader, save_path=best_model_dir)
